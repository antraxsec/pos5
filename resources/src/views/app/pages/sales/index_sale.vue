<template>
	<div class="main-content">
		...
		<div v-else>
			<vue-good-table mode="remote" :columns="columns" :totalRows="totalRows" :rows="sales"
		...
		clearSelectionText: '',
			...
			prevLabel: 'prev',
				...
				<div slot="table-actions" class="mt-2 mb-3">
					...
					<vue-excel-xlsx class="btn btn-sm btn-outline-danger ripple m-1 d-none" :data="sales"
						:columns="columns" :file-name="'ventas'" :file-type="'xlsx'" :sheet-name="'sales'">
						<i class="i-File-Excel"></i> EXCEL
					</vue-excel-xlsx>
					<router-link class="btn-lg btn btn-primary custom-button ripple btn-icon m-1"
				...
				<template slot="table-row" slot-scope="props">
					<span v-if="props.column.field == 'actions'">
						<div>
							<b-dropdown id="dropdown-right" variant="link" text="right align"
								toggle-class="text-decoration-none" size="lg" right no-caret>
								...
								<b-dropdown-item title="Sell Return"
								...
								<b-dropdown-item v-if="currentUserPermissions.includes('payment_sales_view')"
								...
								<b-dropdown-item v-if="currentUserPermissions.includes('shipment')"
								...
								<b-dropdown-item title="Delete" v-if="currentUserPermissions.includes('Sales_delete')"
					...
					<div v-else-if="props.column.field == 'payment_status'">
						<span v-if="props.row.payment_status == 'paid'" class="badge badge-outline-success">{{
							$t('Paid') }}</span>
						<span v-else-if="props.row.payment_status == 'partial'" class="badge badge-outline-primary">{{
							$t('partial') }}</span>
						<span v-else class="badge badge-outline-warning">{{ $t('Unpaid') }}</span>
					</div>
					<div v-else-if="props.column.field == 'shipping_status'">
						<span v-if="props.row.shipping_status == 'ordered'" class="badge badge-outline-warning">{{
							$t('Ordered') }}</span>

						<span v-else-if="props.row.shipping_status == 'packed'" class="badge badge-outline-info">{{
							$t('Packed') }}</span>
						...
						<span v-else-if="props.row.shipping_status == 'cancelled'" class="badge badge-outline-danger">{{
							$t('Cancelled') }}</span>
					</div>
					<div v-else-if="props.column.field == 'Ref'">
						...
						<small v-if="props.row.sale_has_return == 'yes'"><i
								class="text-15 text-danger i-Back"></i></small>
				...
				<b-row>
					...
					<b-col md="12">
						<b-form-group :label="$t('Customer')">
							<v-select :reduce="label => label.value" :placeholder="$t('Choose_Customer')"
					...
					<b-col md="12">
						<b-form-group :label="$t('warehouse')">
							<v-select v-model="Filter_warehouse" :reduce="label => label.value"
					...
					<b-col md="12">
						<b-form-group :label="$t('PaymentStatus')">
							<v-select v-model="Filter_Payment" :reduce="label => label.value"
								:placeholder="$t('Choose_Status')" :options="[
					...
					<b-col md="12">
						<b-form-group :label="$t('Shipping_status')">
							<v-select v-model="Filter_shipping" :reduce="label => label.value"
								:placeholder="$t('Seleccione_Estado')" :options="[
					...
					<b-col md="6" sm="12">
						<b-button @click="Get_Sales(serverParams.page)" variant="primary btn-block ripple m-1"
							size="sm">
							<i class="i-Filter-2"></i>
							{{ $t("Filter") }}
						</b-button>
								...
								<tr v-for="payment in payments">
									...
									<td>
										<div role="group" aria-label="Basic example" class="btn-group">
											...
											<span v-if="currentUserPermissions.includes('payment_sales_edit')"
												title="Edit" class="btn btn-icon btn-success btn-sm"
												@click="Edit_Payment(payment)">
												<i class="i-Pen-2"></i>
											</span>
											<span title="Email" class="btn btn-icon btn-primary btn-sm"
												@click="Send_Email_Payment(payment.id)">
												<i class="i-Envelope"></i>
											</span>
											<span title="SMS" class="btn btn-icon btn-secondary btn-sm"
												@click="Payment_Sale_SMS(payment.id)">
												<i class="i-Speach-Bubble"></i>
											</span>
											<span v-if="currentUserPermissions.includes('payment_sales_delete')"
												title="Delete" class="btn btn-icon btn-danger btn-sm"
		...
		<validation-observer ref="Add_payment">
			<b-modal hide-footer size="lg" id="Add_Payment"
					...
					<b-row>
						...
						<b-col lg="4" md="12" sm="12">
							<validation-provider name="date" :rules="{ required: true }" v-slot="validationContext">
								<b-form-group :label="$t('date')">
									<b-form-input label="date" :state="getValidationState(validationContext)"
										aria-describedby="date-feedback" v-model="payment.date"
						...
						<b-col lg="4" md="12" sm="12">
							<b-form-group :label="$t('Reference')">
								<b-form-input disabled="disabled" label="Reference" :placeholder="$t('Reference')"
									v-model="payment.Ref"></b-form-input>
							</b-form-group>
						</b-col>
							...
							<validation-provider name="Opción de Pago" :rules="{ required: true }">
								<b-form-group slot-scope="{ valid, errors }" :label="$t('OpciónDePago')">
									<v-select :class="{ 'is-invalid': !!errors.length }"
										...
										@input="Selected_PaymentMethod" :disabled="EditPaiementMode"
										:reduce="label => label.value" :placeholder="$t('Seleccione')" :options="[
						...
						<!-- Received  Amount  -->
						<b-col lg="4" md="12" sm="12">
							<validation-provider name="Received Amount"
								:rules="{ required: true, regex: /^\d*\.?\d*$/ }" v-slot="validationContext">
								<b-form-group :label="$t('Received_Amount')" class="font-weight-400"
									style="font-size: 1.25rem;">
									<b-form-input @input="syncPayingAmount"
						...
						<b-col lg="4" md="12" sm="12">
							<validation-provider name="Amount" :rules="{ required: true, regex: /^\d*\.?\d*$/ }"
								...
								<b-form-group :label="$t('Paying_Amount')" class="font-weight-400"
									style="font-size: 1.25rem;">
						...
						<b-col lg="4" md="12" sm="12">
							...
							<p class="change_amount">{{ parseFloat(payment.received_amount -
								payment.montant).toFixed(2)}}</p>
						</b-col>
							...
							<b-card v-show="payment.Reglement == 'credit card' && !EditPaiementMode">
								...
								<div v-if="displaySavedPaymentMethods && !submit_showing_credit_card">
									...
									<table class="table table-hover mt-3">
										...
										<tbody>
											<tr v-for="card in savedPaymentMethods"
					...
					<b-row>
						...
						<b-col md="12">
							<b-form-group :label="$t('delivered_to')">
								<b-form-input label="delivered_to" v-model="shipment.delivered_to"
									:placeholder="$t('delivered_to')"></b-form-input>
							</b-form-group>
						</b-col>
						...
						<b-col md="12" class="mt-3">
							<b-button variant="primary" type="submit" :disabled="Submit_Processing_shipment"><i
									class="i-Yes me-2 font-weight-bold"></i> {{ $t('submit') }}</b-button>
		...
		<b-modal hide-footer size="sm" scrollable id="Show_invoice" :title="$t('Invoice_POS')">
			<div id="invoice-POS">
				<div style="max-width:400px;margin:0px auto">
					...
					<table class="change mt-3 d-none" style=" font-size: 10px;"
						...
						<tbody>
							<tr v-for="payment_pos in payments">
								...
								<td style="text-align: center;" colspan="2">{{ formatNumber(payment_pos.montant, 2) }}
								</td>
								<td style="text-align: right;" colspan="1">{{ formatNumber(payment_pos.change, 2) }}
								</td>
							</tr>
						</tbody>
					</table>
					...
					<div id="legalcopy" class="text-center w-100 mt-4">
						...
						<div id="bar" v-show="pos_settings.show_barcode" class="invoice_logo text-center mb-2">
							<barcode class="barcode" :format="barcodeFormat" :value="invoice_pos.sale.Ref"
								textmargin="0" fontoptions="bold" fontSize="15" height="25" width="1"></barcode>
						</div>
					</div>
				</div>
			</div>
